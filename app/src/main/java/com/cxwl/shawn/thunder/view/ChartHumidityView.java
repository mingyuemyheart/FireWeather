 ...
 * @author shawn_sun
 */
public class ChartHumidityView extends View {
	...
	public ChartHumidityView(Context context) {
		super(context);
		mContext = context;
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (!TextUtils.isEmpty(dto.getHumidity()) && !TextUtils.equals(dto.getHumidity(), CONST.noValue)) {
				float value = Float.valueOf(dto.getHumidity());
				if (value >= 0) {
		...
		canvas.drawText("(%)", w-CommonUtil.dip2px(mContext, 40f), CommonUtil.dip2px(mContext, 10f), textP);
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (i % 2 == 0) {
				if (!TextUtils.isEmpty(dto.getHumidity()) && !TextUtils.equals(dto.getHumidity(), CONST.noValue)) {
					...
					textP.setTextSize(CommonUtil.dip2px(mContext, 10));
					float tempWidth = textP.measureText(dto.getHumidity());
					canvas.drawText(dto.getHumidity(), dto.getX()-tempWidth/2, dto.getY()-CommonUtil.dip2px(mContext, 10), textP);
				}
