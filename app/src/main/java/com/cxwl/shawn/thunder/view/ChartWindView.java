 ...
 * @author shawn_sun
 */
public class ChartWindView extends View {
	...
	public ChartWindView(Context context) {
		super(context);
		mContext = context;
		init();
	}
	...
	public void setData(ArrayList<StationDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			String temp = tempList.get(0).getWindSpeed();
			for (int i = 0; i < tempList.size(); i++) {
				StationDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.getWindSpeed()) && !TextUtils.equals(dto.getWindSpeed(), CONST.noValue)) {
					temp = dto.getWindSpeed();
					break;
				}
			}
			if (!TextUtils.isEmpty(temp) && !TextUtils.equals(temp, CONST.noValue)) {
				...
				for (int i = 0; i < tempList.size(); i++) {
					StationDto dto = tempList.get(i);
					if (!TextUtils.isEmpty(dto.getWindSpeed()) && !TextUtils.equals(dto.getWindSpeed(), CONST.noValue)) {
						if (maxValue <= Float.valueOf(dto.getWindSpeed())) {
							maxValue = Float.valueOf(dto.getWindSpeed());
						}
						if (minValue >= Float.valueOf(dto.getWindSpeed())) {
							minValue = Float.valueOf(dto.getWindSpeed());
						}
					}
				}
			}
...
//				minValue = (float) (Math.floor(minValue)-itemDivider);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (!TextUtils.isEmpty(dto.getWindSpeed()) && !TextUtils.equals(dto.getWindSpeed(), CONST.noValue)) {
				float value = Float.valueOf(dto.getWindSpeed());
				if (value >= 0) {
		...
		canvas.drawText("(m/s)", w-CommonUtil.dip2px(mContext, 40f), CommonUtil.dip2px(mContext, 10f), textP);
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (i % 2 == 0) {
				if (!TextUtils.isEmpty(dto.getWindSpeed()) && !TextUtils.equals(dto.getWindSpeed(), CONST.noValue)) {
					...
					textP.setTextSize(CommonUtil.dip2px(mContext, 10));
					float tempWidth = textP.measureText(dto.getWindSpeed());
					canvas.drawText(dto.getWindSpeed(), dto.getX()-tempWidth/2, dto.getY()-CommonUtil.dip2px(mContext, 10), textP);
				}
