 ...
 * @author shawn_sun
 */
public class ChartPressureView extends View {
	...
	public ChartPressureView(Context context) {
		super(context);
		mContext = context;
		init();
	}
	...
	public void setData(ArrayList<StationDto> dataList) {
		if (!dataList.isEmpty()) {
			for (int i = 0; i < dataList.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue) && !TextUtils.equals(dto.getPressure(), "0")) {
			...
			String temp = tempList.get(0).getPressure();
			for (int i = 0; i < tempList.size(); i++) {
				StationDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
					temp = dto.getPressure();
					break;
				}
			}
			if (!TextUtils.isEmpty(temp) && !TextUtils.equals(temp, CONST.noValue)) {
				...
				for (int i = 0; i < tempList.size(); i++) {
					StationDto dto = tempList.get(i);
					if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
						if (min >= Float.valueOf(dto.getPressure())) {
							min = Float.valueOf(dto.getPressure());
						}
					}
				}
				...
				for (int i = 0; i < tempList.size(); i++) {
					StationDto dto = tempList.get(i);
					if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
						float air = Float.valueOf(dto.getPressure());
						dto.setPressure(String.valueOf(air - min));
					}
				}
			}

			String temp2 = tempList.get(0).getPressure();
			for (int i = 0; i < tempList.size(); i++) {
				StationDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
					temp = dto.getPressure();
			...
			if (!TextUtils.isEmpty(temp2) && !TextUtils.equals(temp2, CONST.noValue)) {
				...
				for (int i = 0; i < tempList.size(); i++) {
					StationDto dto = tempList.get(i);
					if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
						if (maxValue <= Float.valueOf(dto.getPressure())) {
							maxValue = Float.valueOf(dto.getPressure());
						}
						if (minValue >= Float.valueOf(dto.getPressure())) {
							minValue = Float.valueOf(dto.getPressure());
						}
					}
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
				float value = Float.valueOf(dto.getPressure());
				if (value >= 0) {
		...
		canvas.drawText("(hPa)", w-CommonUtil.dip2px(mContext, 40f), CommonUtil.dip2px(mContext, 10f), textP);
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (i % 2 == 0) {
				if (!TextUtils.isEmpty(dto.getPressure()) && !TextUtils.equals(dto.getPressure(), CONST.noValue)) {
					...
					textP.setTextSize(CommonUtil.dip2px(mContext, 10));
					float tempWidth = textP.measureText(String.valueOf(Float.valueOf(dto.getPressure())+min));
					canvas.drawText(String.valueOf(Float.valueOf(dto.getPressure())+min), dto.getX()-tempWidth/2, dto.getY()-CommonUtil.dip2px(mContext, 10), textP);
				}
