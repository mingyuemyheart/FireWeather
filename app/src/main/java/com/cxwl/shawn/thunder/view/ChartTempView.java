 ...
 * @author shawn_sun
 */
public class ChartTempView extends View {
	...
	public ChartTempView(Context context) {
		super(context);
		mContext = context;
		init();
	}
	...
	public void setData(ArrayList<StationDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			String temp = tempList.get(0).getTemp();
			for (int i = 0; i < tempList.size(); i++) {
				StationDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.getTemp()) && !TextUtils.equals(dto.getTemp(), CONST.noValue)) {
					temp = dto.getTemp();
					break;
				}
			}
			if (!TextUtils.isEmpty(temp) && !TextUtils.equals(temp, CONST.noValue)) {
				...
				for (int i = 0; i < tempList.size(); i++) {
					StationDto dto = tempList.get(i);
					if (!TextUtils.isEmpty(dto.getTemp()) && !TextUtils.equals(dto.getTemp(), CONST.noValue)) {
						if (maxValue <= Float.valueOf(dto.getTemp())) {
							maxValue = Float.valueOf(dto.getTemp());
						}
						if (minValue >= Float.valueOf(dto.getTemp())) {
							minValue = Float.valueOf(dto.getTemp());
						}
					}
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (!TextUtils.isEmpty(dto.getTemp()) && !TextUtils.equals(dto.getTemp(), CONST.noValue)) {
				float value = Float.valueOf(dto.getTemp());
				if (value >= 0) {
		...
		canvas.drawText("(℃)", w-CommonUtil.dip2px(mContext, 30f), CommonUtil.dip2px(mContext, 10f), textP);
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (i % 2 == 0) {
				if (!TextUtils.isEmpty(dto.getTemp()) && !TextUtils.equals(dto.getTemp(), CONST.noValue)) {
					...
					textP.setTextSize(CommonUtil.dip2px(mContext, 10));
					float tempWidth = textP.measureText(dto.getTemp());
					canvas.drawText(dto.getTemp(), dto.getX()-tempWidth/2, dto.getY()-CommonUtil.dip2px(mContext, 10), textP);
				}
